...
graf_1 = ggplot(iris, aes(Petal.Length, Petal.Width, color = Species)) + geom_point(size = 3)
...
# Plot
clusplot(iris, modelo$cluster, color = TRUE, shade = TRUE, labels = 0, lines = 0, )
View(dados)
dados$cluster <- modelo$cluster
graf_2 <- ggplot(dados, aes(x = Petal.Length, y = Petal.Width, color = factor(cluster))) + geom_point() + theme_minimal()
